...
class Waypoint:
    ...
    def market(self):
        return self.client.systems.waypoints.market(self.system_symbol, self.symbol)

    def shipyard(self):
        return self.client.systems.waypoints.shipyard(self.system_symbol, self.symbol)

    def jumpgate(self):
        return self.client.systems.waypoints.jumpgate(self.system_symbol, self.symbol)

    def distance(self, destination):
        return math.sqrt((destination.x - self.x) ** 2 + (destination.y - self.x) ** 2)

    def connected_systems(self):
        return self.jumpgate().get("connectedSystems")
